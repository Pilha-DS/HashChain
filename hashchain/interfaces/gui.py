...
def on_closing():
    """Trata o fechamento da janela de forma segura."""
    # Flag global para indicar que estamos fechando
    global _closing
    _closing = True
    
    try:
        # Cancela todos os callbacks pendentes do Tkinter
        try:
            ...
            if after_ids:
                for after_id in after_ids:
                    try:
                        ...
                        # Ignora erros de callbacks já executados ou inválidos
                        pass
        except Exception:
            pass
        
        # Para o loop principal antes de destruir
        try:
            root.quit()
        except:
            pass
        
        # Pequeno delay para garantir que o quit foi processado
        try:
            root.update()
        except:
            pass
...
# Flag para controlar o estado de fechamento
_closing = False
...
def interface_menu():
    ...
    ctk.CTkButton(content_frame, text="Voltar ao terminal", command=on_closing).pack(pady=10, padx=10)
