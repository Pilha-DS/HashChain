"""
Script para construir o executável .exe do HashChain.
...
def build_exe():
    ...
    print("🔨 Iniciando construção do executável HashChain...")
    print("=" * 60)
    ...
    # Comando PyInstaller
    # Usa --console=False mas o código detecta e abre GUI automaticamente
    cmd = [
        "pyinstaller",
        "--name=HashChain",
        "--onefile",  # Arquivo único
        "--windowed",  # Sem console (para GUI) - código detecta e abre GUI automaticamente
    ...
    # Adiciona dados adicionais (arquivos não-Python)
    if (project_root / "config.json").exists():
        config_path = project_root / "config.json"
        cmd.extend(["--add-data", f"{config_path}{os.pathsep}."])
    
    # Inclui templates da web se existirem
    web_templates = project_root / "hashchain" / "interfaces" / "web" / "templates"
    if web_templates.exists():
        cmd.extend(["--add-data", f"hashchain/interfaces/web/templates{os.pathsep}hashchain/interfaces/web/templates"])
    ...
    cmd.extend([
        ...
        "--collect-all", "customtkinter",  # Coleta recursos do customtkinter
    ])
